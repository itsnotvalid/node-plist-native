...
var toPlist = function(doc, object, parent) {
  ...
  case 'object':
    if(object instanceof Buffer) {
      element = new libxml.Element(doc, 'data').text(object.toString('base64'));
    } else if(object instanceof Date) {
      element = new libxml.Element(doc, 'date').text(object.toISOString().substr(0, 19) + 'Z');
    } else if(Array.isArray(object)) {
      element = new libxml.Element(doc, 'array');
      object.forEach(function(node) { toPlist(doc, node, element); });
    } else {
      if (Object.prototype.hasOwnProperty.call(object, 'type') && object.type === 'Buffer') {
        const b = Buffer.from(object.data);
        // we have serialized object
        element = new libxml.Element(doc, 'data').text(b.toString('base64'));
      } else {
        element = new libxml.Element(doc, 'dict');
    ...
    break;
  case 'string':
    element = new libxml.Element(doc, 'string').text(object);
    break;
  case 'number':
    element = new libxml.Element(doc, object % 1 === 0 ? 'integer' : 'real').text(object);
    break;
  case 'boolean':
    element = new libxml.Element(doc, object.toString());
    break;
  default:
...
var toObject = function(node) {
  ...
  case 'data':
    return Buffer.from(node.text(), 'base64');
  case 'date':
...
exports.parseString = function(xml) {
  var xmlDoc = libxml.parseXml(xml, {recover: true});
...
exports.build = function(data, formatted) {
  return Buffer.from(exports.buildString(data, formatted));
};
